...
void main() {
  ...
  group('ChatViewModel', () {
    ...
    test('sendMessage sends message and updates db', () async {
      ...
      when(() => mockAiService.sendMessage(any(), any())).thenAnswer(
        (_) async => {'success': true, 'text': 'Hello from AI!'},
      );
      ...
      verify(() => mockDbHelper.insertMessage('model', 'Hello from AI!')).called(1);
    ...
    test('sendMessage handles error gracefully', () async {
      ...
      when(() => mockAiService.sendMessage(any(), any())).thenAnswer(
        (_) async => {'success': false, 'error': 'API Error'},
      );

      await viewModel.sendMessage('Hello!');

      verify(() => mockDbHelper.insertMessage('user', 'Hello!')).called(1);
      verify(() => mockDbHelper.insertMessage('system', 'Error: API Error')).called(1);
      expect(viewModel.isLoading, isFalse);
    });
